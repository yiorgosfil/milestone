...
export default function BoardView({ id, setIsModalNewTaskOpen }: BoardProps) {
  ...
  const moveTask = (taskId: number, toStatus: string) => {
    updateStatus({ taskId, status: toStatus })
  }
...
function TaskColumn ({
  ...
  // This `useDrop` hook enables a component to act as a drop target 
  // for draggable items of type `task`
  ...
  return (
    ...
    >
      {task.attachments && task.attachments.length > 0 && (
        <Image
          src={`todo/${task.attachments[0].fileUrl}`}
          ...
          <div className="flex -space-x-[6px] overflow-hidden">
            {task.assignee && (
              <Image
                ...
                src={`todo/${task.assignee.profilePictureUrl!}`}
            ...
            {task.author && (
              <Image
                ...
                src={`todo/${task.author.profilePictureUrl!}`}
